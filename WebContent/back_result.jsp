...
<meta http-equiv="refresh" content ="3;url=borrow_log.jsp?id_number=<%=id_number%>">
</head>
<body>
...
u_back="update borrow set back_time ='"+dates+"'where id_number="+id_number+" and book_isbn="+isbn+" and borrow_time='"+borrow_date+"';";
rs_u_back=st.executeUpdate(u_back);
if(rs_u_book!=0 && rs_u_back!=0){%>
<h1>操作成功！</h1>
<h1><span id="sp">5</span>秒钟后自动跳转到借阅记录页面！！！</h1>
<h1>如果没有跳转，请点击<a href="borrow_log.jsp?id_number=<%=id_number %>">这里</a></h1>
<%rs_isbn.close();
...
<h1><span id="sp">5</span>秒钟后自动跳转到借阅记录页面！！！</h1>
<h1>如果没有跳转，请点击<a href="boorow_log.jsp?id_number=<%=id_number %>">这里</a></h1>
<%}%>
</div>
<div id="footer">
